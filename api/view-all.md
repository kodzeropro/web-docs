...
### Типы поля: Текст, Email, URL, Контент
...
::: tip :warning: Подсказка
Фильтрация по Дате и времени пока недоступна. Пожалуйста, напишите на team@kodzero.pro или https://t.me/leshatour, если вам необходима эта возможность.
:::
...
### Тип поля: Связь (Relation ID)

Для полей типа **Связь** поддерживается поиск по одному идентификатору или по нескольким через запятую (логическое ИЛИ).

```http
# Поиск по одному связующему ID
GET /v1/:project/:collection?category=6a33b602ad84ff6b9de74710

# Поиск по нескольким ID (ИЛИ)
GET /v1/:project/:collection?category=6a33b602ad84ff6b9de74710,6a2932b7ed6073c9e39a3d89
```
...
- Более гибкие настройки фильтров находятся в разработке:
  - `!=` — не содержит `значение`
  - `>`, `>=`, `<`, `<=` — больше, меньше, чем `значение`
