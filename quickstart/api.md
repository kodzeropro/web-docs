...
<CrossLink section="REST API" page="Distinct" full-url='/docs/api/distinct' />

Получение уникальных значений по полю. Полезно для построения фильтров (например, список всех брендов в поле `brand`).

```http
GET /v1/:project/:collection/distinct?field=brand
```

В настройках API для метода `Distinct` можно:
- включить/выключить метод,
- задать уровень доступа,
- указать ограничение полей, по которым разрешён запрос.
...
Для публичных данных используйте `Public` только на методах `VIEW ALL`, `VIEW`, `DISTINCT`. Методы `CREATE`, `UPDATE`, `DELETE` лучше защитить уровнем `User` или `Workspace`.
:::
